...
--TODO figure out how to prevent the case of 7 1 2 3 failing, yet 8 9 1 2 failing!
...
isSafe t exceptions (a:b:xs) = let 
  ...
  sameTrajectory = case t of
    ...
    Decreasing -> a > b
  in
    if not safe || not sameTrajectory then 
      isSafe t (exceptions-1) (a:xs)
    else 
      isSafe t exceptions (b:xs)
